...
// Invalid pointers, dimensions (including N != K + P), check-matrix indices, or
// convergence only at the iteration cap are reported as nIteration_max + 1.
// Invalid inputs are rejected before LLRo is modified.
int decode_GBF(
		const float LLRi[],
		int LLRo[],
		const int* C,
