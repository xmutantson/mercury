...
// =============================================================================
// K-GENERALIZED data-frame variant (SIM/RESEARCH only — robust3-gf16ra)
// =============================================================================
//
// fact-documents/robust3-gf16ra-data-code-feasibility.md. The functions above
// are hard-wired to K=13 (the 40-bit message + 12-bit CRC ctrl suffix) and stay
...
// WHT Q-ary BP) to an ARBITRARY number of info symbols, to test whether the
// non-binary RA code scales to ROBUST_3's data-frame payload (K_info ≈ 200
// GF(16) symbols = 800 info bits) while staying near-capacity, or is inherently
// a short-message code (the make-or-break scaling question).
//
// These are NOT wired into any production path. They operate on raw GF(16)
// symbol vectors (no [type|payload|crc] packing, no CRC accept-gate — the ARQ
// layer's frame CRC is the integrity check for data; see fact-doc §5.3). They
// allocate per-call on the heap so K/N are unbounded by GF16RA_MAX_N.
...
// info to score symbol/frame errors (research harness) or, in production, would
// repack to bits and let the ARQ CRC verify.
