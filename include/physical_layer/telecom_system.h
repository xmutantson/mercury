...
struct st_receive_stats{
	...
	bool tinterp_rescue_attempted = false;  // current receive_byte call reached the LS->TIME_INTERP retry
	bool tinterp_rescue_succeeded = false;  // that retry, rather than the primary estimator, decoded
...
public:
	...
	// Deterministic receive-path test hook. A negative value is production/default;
	// otherwise the next OFDM estimator pass sees exactly this many valid pilot
	// symbols, with the sequence restored before downstream processing.
	int selectivity_test_valid_pilots = -1;
