...
public:
  ...
  // sample the MFSK suffix (arq_common.cc receive_mfsk_ctrl_suffix_phy_core),
  // but the OFDM data consumer (this->receive()) needs frames_to_read =
  // frame_symb+10 to capture a full data frame. They share ONE frames_to_read
  // and receive() runs the very next line. The original block re-clamped ftr=2
  // EVERY pre-data iteration, so the data frame never decoded,
  // batch_rx_frame_count never advanced, and the pre-data window never closed
  // (the INV-B self-terminate was a dead invariant). FIX = WINDOW-BOUNDED
  // OWNERSHIP: the probe owns ftr only for a short turnaround sub-window after
  // entering the CONNECTED pre-data RECEIVING window (the lost-ACK duplicate
  // arrives within ~1 CMD retransmit cycle). After the window elapses we hand
  // ftr back to the OFDM data path ONCE (a one-shot, so the capture thread's
  // drain is not fought every tick) and the probe never clamps again -> OFDM
  // delivery restored, heal preserved.
