...
public:
  ...
  // LINK-PHASE PRIMITIVE (increment 1) directed shadow-agreement unit. Synthetic-fires the
  // production producers on a throwaway controller and asserts the pre-init owner==NONE guard,
  // the owner ordering, the DIRECTIONAL over-wait/under-estimate meter (incl. the intended
  ...
  // (CLI --test-recovery-ack-capture). Constructs the measured HW mis-phase
  // (a recovery-ACK at a LATE ring phase abutting the right tail edge, and an
  // unwritten exact-zero ring head) and asserts: fail-before the newest-tail
  // snapshot scores the truncated/absent buffer (present-run ~3, matched<7);
  // pass-after the multi-window look-back finds the FULL block at an older
  ...
  // plateau/gap the grid is torn down to force a clean preamble re-acquire past
  // the coasted region. N (default arm 8) is chosen ABOVE the benign shallow
  // over-read depth so a genuine sparse-fade multi-frame coast (the reason the
  // coast exists) still runs, and well BELOW the pathological deep depth. Env-
  // latched in the ctor like batch_coast_defeat. batch_coast_stops counts the
  // forced re-acquires (emitted stops=%lld on every [BATCH-COAST-STOP] line).
  int batch_coast_max;
  long long batch_coast_stops;
