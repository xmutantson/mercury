...
// Topgear telemetry extension of the compact confirm. The first 16+10 symbols
// are generated by the legacy routine verbatim. A second K=5 codeword then
// carries [report:8|crc12(report):12], continuing the same absolute tone hop.
void cl_mfsk::generate_topgear_confirm_pattern(std::complex<double>* pattern_out,
	uint8_t bsi, uint16_t crc12, uint8_t report, uint16_t report_crc12)
{
	generate_compact_confirm_pattern(pattern_out, bsi, crc12);
	int suffix_len = compact_confirm_suffix_len();
	if(suffix_len == 0 || M == 0 || Nc == 0 || nStreams == 0) return;

	int report_tones[MAX_ACK_SACK_SUFFIX];
	gf16ra::encode_compact(report, report_crc12, report_tones);
	double amp = sqrt((double)Nc / nStreams);
	for(int s = 0; s < suffix_len; s++)
	{
		int abs_s = ack_pattern_nsymb + suffix_len + s;
		for(int k = 0; k < Nc; k++)
			pattern_out[abs_s * Nc + k] = std::complex<double>(0.0, 0.0);
		int actual_tone = (report_tones[s] + abs_s * tone_hop_step) % M;
		for(int st = 0; st < nStreams; st++)
			pattern_out[abs_s * Nc + stream_offsets[st] + actual_tone] =
				std::complex<double>(amp, 0.0);
	}
