...
int main() {
	...
	if (n != 10) { fprintf(stderr, "expected 10 tones at M=16, got %d\n", n); return 1; }
	...
	for (int trial = 0; trial < 10000; trial++) {
		...
		int tones[16] = {0};
		int wrote = m.pack_ack_sack_payload(bsi, bitmap, tones);
		if (wrote != n) { fprintf(stderr, "pack returned %d\n", wrote); n_fail++; continue; }
		uint8_t  rx_bsi    = 0;
		uint32_t rx_bitmap = 0;
		bool ok = m.unpack_ack_sack_payload(tones, &rx_bsi, &rx_bitmap);
		if (!ok || rx_bsi != bsi || rx_bitmap != bitmap) {
			fprintf(stderr, "FAIL trial=%d bsi=%02x bitmap=%08x -> rx_bsi=%02x rx_bitmap=%08x\n",
			        trial, bsi, bitmap, rx_bsi, rx_bitmap);
			n_fail++;
		} else {
			n_ok++;
		}
