	...
	int interpolation_rate, int preamble_nSymb)
{
	...
	// DIAG: per-symbol breakdown at best coarse position. Gated on g_verbose:
	// time_sync_preamble_matched is now on the per-frame production RX hot path
	// (SIM-validated fine-timing swap, 2026-06-04), so this must not spam logs
	// / add latency on every sub-threshold fine slice (cf. printf-in-hot-loop
	// SACK bug #15). Available with -v for debugging.
	...
	// Gated on g_verbose (per-frame production RX hot path; see [MF-DIAG] above).
